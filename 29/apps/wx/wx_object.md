# `wx_object`
[🔗](https://github.com/erlang/otp/blob/OTP-29.1.1/lib/wx/src/wx_object.erl#L30)
...
```erlang
-spec start(Name, Mod, Args, Options) -> wxWindow:wxWindow() | ignore | {error, term()}
               when
                   Name :: {local, atom()},
                   Mod :: atom(),
                   Args :: term(),
...
```erlang
-spec start_link(Mod, Args, Options) -> wxWindow:wxWindow() | ignore | {error, term()}
                    when
                        Mod :: atom(),
                        Args :: term(),
                        Flag :: trace | log | {logfile, string()} | statistics | debug,
...
```erlang
-spec start_link(Name, Mod, Args, Options) -> wxWindow:wxWindow() | ignore | {error, term()}
                    when
                        Name :: {local, atom()},
                        Mod :: atom(),
                        Args :: term(),
