.PHONY: setup clean build update serve test algolia setup_gems setup_npm format-eeps patches assets/img/favicon.ico build-docs
...
docs/LATEST_MAJOR_VSN: otp_versions.table
	@mkdir -p $(@D)
	@set -e ;\
...
docs: otp_versions.table _scripts/download-docs.sh _scripts/otp_flatten_docs _scripts/otp_flatten_ex_docs _scripts/otp_doc_sitemap.sh docs/LATEST_MAJOR_VSN _scripts/otp_add_headers.sh
	if [ ! -d $@ ]; then git clone --single-branch -b $@ https://github.com/erlang/erlang-org $@; fi
...
otp-headers: docs _redirects _scripts/otp_add_headers.sh docs/LATEST_MAJOR_VSN
