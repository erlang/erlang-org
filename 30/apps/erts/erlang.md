# `erlang`
[🔗](https://github.com/erlang/otp/blob/master/erts/preloaded/src/erlang.erl#L22)
...
```erlang
-type fun_info_item() :: arity | env | index | name | module | new_index | new_uniq | type | uniq.
```
...
```erlang
-spec pid_to_binary(Pid) -> binary() when Pid :: pid().
```
...
```erlang
-spec port_to_binary(Port) -> binary() when Port :: port().
```
...
```erlang
-spec ref_to_binary(Ref) -> binary() when Ref :: reference().
```
...
> This function is deprecated. erlang:exit/2 is deprecated; use erlang:exit_signal/2 instead.
...
> This function is deprecated. erlang:exit/3 is deprecated; use erlang:exit_signal/3 instead.
