...
```erlang
-spec c(Module) -> {ok, ModuleName} | ok | error | {error, Reason :: term()}
           when Module :: file:name(), ModuleName :: module().
```

# `c`

```erlang
-spec c(Module, Options) -> {ok, ModuleName} | ok | error | {error, Reason :: term()}
           when
               Module :: file:name(),
...
```erlang
-spec c(Module, Options, Filter) -> {ok, ModuleName} | ok | error | {error, Reason :: term()}
           when
               Module :: atom(),
