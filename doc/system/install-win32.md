...
version of Erlang, for example, is `otp_win64_29.exe`.
...
*   Modify PATH and other environment variables so that all these tools
    ...
    first row and change `otp_win64_29` to `otp_win32_29` on
    the last row):
        ...
        $ release/win32/otp_win64_29 /S

Voila! `Start->Programs->Erlang OTP 29->Erlang` starts the Erlang
Windows shell.
...
*   Get the Erlang source distribution (from <http://www.erlang.org/download.html>).
    ...
    unpack the source tar.gz (`tar zxf otp_src_29.tar.gz`) to somewhere
    ...
    `otp_src_29.tar.gz`, I then add the following to `.profile`:
    
    ```text
    ERL_TOP=/mnt/c/src/otp_src_29
    export ERL_TOP
    ```
...
Now you will have a file called `otp_win32_29.exe` or `otp_win64_29.exe`
...
4.  `$ ./otp_build installer_win32` - Creates the self extracting installer executable.
    The executable `otp_win32_29.exe` or `otp_win64_29.exe` will be placed
    ...
    and after a while Erlang/OTP-29 will have been installed in
...
doing this, preferably the plain 29 you have built in the previous
...
*   Q: Which version of WSL and other tools do you use then?
    ...
    The GCC we used for 29 was version 7.3-win32.
