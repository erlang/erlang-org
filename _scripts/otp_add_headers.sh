...
DOCS_DIR="$1"
shift
LATEST_MAJOR_VSN="$(cat "${DOCS_DIR}/LATEST_MAJOR_VSN")"
MAJOR_VSNS="$(ls -I "*\.*" -I "LATEST_MAJOR_VSN" ${DOCS_DIR})"
...
_add_canonical() {
    ...
    for file in "$@"; do
        ...
        if [ ! $MAJOR_VSN -gt "26" ]; then
            CURR_FILENAME=${canonical/#apps\/*\//}
            REDIRECT=$(cat "_redirects" | grep -P "^/doc/.*?/?${canonical#apps\/}" || echo "")
...
for MAJOR_VSN in ${MAJOR_VSNS}; do
    ...
    _add_head_tags $(find ${TARGET_DIR} -type f -name "*.html" -print0 | xargs -0 echo $1 | awk -F"\n" '{print $1}')
